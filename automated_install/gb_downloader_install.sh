...
InstallPackagesRequiredForInstallScript() {
    ...
    apt-get update
    apt-get install -y ${INSTALLER_DEPS[@]}
...
InstallPackagesRequiredForGiantbombDownloader() {
    ...
    apt-get install -y ${GB_DOWNLOADER_DEPS[@]}
}
...
ConfigureApache() {
    ...
    cp /var/www/giantbomb_downloader/automated_install/configs/apache2/giantbomb_downloader.conf /etc/apache2/sites-available/giantbomb_downloader.conf
    a2enconf php7.1-fpm
...
CreateEnvFile() {
    ...
    echo "DB_PASSWORD=${DATABASE_PASSWORD}" >> "/var/www/giantbomb_downloader/.env"
    echo "APP_KEY=" >> "/var/www/giantbomb_downloader/.env"
}

ConfigureSupervisor() {

    supervisorctl stop all
    cp -R /var/www/giantbomb_downloader/automated_install/configs/supervisor/* /etc/supervisor/conf.d/
...
SetupLaravelFramework() {
    ...
    chmod 777 -R /var/www/giantbomb_downloader/storage/
    php /var/www/giantbomb_downloader/artisan key:generate
    php /var/www/giantbomb_downloader/artisan migrate
}
